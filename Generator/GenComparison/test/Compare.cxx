{
 ...
 std::string FolderImages = "images_MINLO_HERWIGpp_Pythia";
...
//  std::vector < TCanvas > vCanvas;
 ...
 vFile.push_back("WH_Herwig.root");
 vFile.push_back("WH_Minlo.root");
 vFile.push_back("WH_Pythia.root");
...
//  vSampleWeight.push_back("baseW*(Vdecay==-2)"); //---- to remove W-
 vSampleWeight.push_back("baseW");
 vSampleWeight.push_back("baseW");
//  vSampleWeight.push_back("baseW*(Vdecay==-2)"); //---- to remove everything except W+>munu
 vSampleWeight.push_back("baseW");
//  vSampleWeight.push_back("baseW");
//  vSampleWeight.push_back("baseW");
...
//  vCuts.push_back ("abs(mjj-85)>20");
//  vCuts.push_back ("Vdecay==-2"); // ----  W+ > mu,numu
 ...
 vVariableHR.push_back("mjj");
 vVariable.push_back("mjj");
 vVariable_min.push_back(-20);
 vVariable_max.push_back(200);
 vVariable_bin.push_back(50);

 vVariableHR.push_back("lhejetpt1");
 vVariable.push_back("lhejetpt1");
 vVariable_min.push_back(-2);
 vVariable_max.push_back(100);
 vVariable_bin.push_back(102);
...
//     }
    ...
    ccNorm->cd(1);
    double int_h = h[iFile]->Integral(0,h[iFile]->GetNbinsX()+1);
    hNorm[iFile] = (TH1F*) h[iFile] -> Clone();
    std::cout << "   >> int_h = " << int_h << std::endl;
    hNorm[iFile] -> Scale (1. / int_h);

    int_h = hNorm[iFile]->Integral(0,hNorm[iFile]->GetNbinsX()+1); //---- also overflow!
    std::cout << "   >> int_h norm = " << int_h << std::endl;
