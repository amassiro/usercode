...
void ReverseCumulative(TH1* histo, double integral_scale) {
 double integral = histo->GetBinContent(histo->GetNbinsX());
 for (int iBin = (histo->GetNbinsX()-1); iBin >= 0; iBin --) {
//   for (int iBin = (histo->GetNbinsX()-1); iBin >= 0; iBin --) {
//   for (int iBin = 0; iBin < histo->GetNbinsX(); iBin ++) {
  double value = histo->GetBinContent(iBin);
  double error = 0;
  
  if (iBin == (histo->GetNbinsX()-1)) {//---- if the last bin
   error = histo->GetBinError(iBin+1);
  }
  else {
   error = integral_scale * (histo->GetBinError(iBin+2));
   error = Qsum (error, histo->GetBinError(iBin+1));
//    for (int jBin = iBin+1; jBin <= (iBin+2); jBin ++) { //---- the others has already been summed!
//    error = Qsum (error, histo->GetBinError(jBin));
//    }
  }
//   if (error == 0) std::cout << " iBin = " << iBin << " [of " << histo->GetTitle() << " ]" << " while iBin+1 = " <<  histo->GetBinError(iBin+1) << std::endl;
  error = error / integral_scale;
  histo->SetBinContent   (iBin+1, integral - value);
  histo->SetBinError     (iBin+1, error); 
 }
}   


void ZeroErrorForZeroEntry(TH1F* histo) { 
 for (int iBin = 0; iBin < histo->GetNbinsX(); iBin ++) {
...
TH1F* IntegralRight(TH1F* histo) { 
 ZeroErrorForZeroEntry(histo);
 histo->ComputeIntegral();
 Double_t *integral = histo->GetIntegral();
 TH1F* tempOfHisto = (TH1F*) histo->Clone();
 tempOfHisto->Sumw2(); //----> important: before "SetContent"
 tempOfHisto->SetContent(integral);
 double integral_scale =  histo->Integral();
 ReverseCumulative(tempOfHisto,integral_scale);
...
float Qsum (float A, float B) {
 return sqrt(A*A + B*B);
}


///---- build percentage histogram from composition histogram ----////
void TransformTS(THStack* hsinput, THStack* hsoutput) {
//  TString name = Form("%s_percentage",hsinput->GetName());
//  hsoutput = new THStack(name,name);
//  std::cout << " &hsoutput = " << hsoutput << std::endl;

 TObjArray* histos = hsinput->GetStack () ;
 Int_t number = histos->GetEntries();
 TH1F* last = (TH1F*) histos->At (number-1) ;
 for (int iHist = 0; iHist < number; iHist++) {
  TH1F * histo = ((TH1F*) histos->At (iHist))->Clone() ;
  if (iHist != 0)  {
   TH1F * prevhisto = (TH1F*) histos->At (iHist-1) ;
   histo->Add(prevhisto,-1);
  }
  histo->Divide(last);
  hsoutput->Add(histo);
 }
...
TLegend * makeLegend (THStack stack, const std::vector<std::string> & nameSample) { 
 ...
 TObject * obj ;
 int i=0;
 std::cout << "legend::(begin)" << std::endl;
 while ((obj = next ())) {
  TString name = Form("%s",nameSample.at(i).c_str());
  leg->AddEntry (obj,name,"f") ;
  i++;
  std::cout << "legend::name = " << name.Data() << std::endl;
 }
 std::cout << "legend::(end)" << std::endl;
 return leg ;
}
...
TH1D* DynamicalRebinHisto ( TH1D * original_Histo, TH1D* rebinned_Histo, std::vector<float>  binning,  TString &nameHisto, bool isDATA, bool isDivide) {
 ...
 if (binning.at(binning.size()-1) > original_Histo->GetXaxis()->GetBinUpEdge(original_Histo->GetNbinsX())) {
  std::cerr << "Error: Invalid Max Bin " ;
  std::cerr << " :: max = " << original_Histo->GetXaxis()->GetBinUpEdge(original_Histo->GetNbinsX());
  std::cerr << " while set to = " << binning.at(binning.size()-1) << std::endl;
  return(0);
 }
