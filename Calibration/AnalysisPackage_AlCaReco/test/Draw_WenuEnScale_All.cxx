...
//  double VALUE_MIN[100] = {0.70,0.75,0.80,0.85,0.90,0.95,0.96,0.97,0.98,0.99};
//  double VALUE_MAX[100] = {0.75,0.80,0.85,0.90,0.95,0.96,0.97,0.98,0.99,1.00};
//  double RANGE_MAX[100] = {2.20,2.00,1.80,1.60,1.40,1.20,1.20,1.20,1.20,1.20};
//  double VALUE_MIN[100] = {0.70,0.75,0.80,0.85,0.90,0.95};
//  double VALUE_MAX[100] = {0.75,0.80,0.85,0.90,0.95,1.00};
//  double RANGE_MAX[100] = {2.20,2.00,1.80,1.60,1.40,1.20};
 
 
   char VARIABLE[1000] = {"eleFBrem"};
   int NUM = 1;
   double VALUE_MIN[100] = {0.00,0.10,0.20,0.30,0.40,0.50,0.60,0.70,0.80,0.90};
   double VALUE_MAX[100] = {0.10,0.20,0.30,0.40,0.50,0.60,0.70,0.80,0.90,1.00};
   double RANGE_MAX[100] = {1.20,1.20,1.20,1.20,1.20,1.20,1.20,1.20,1.20};
...
//   double MaxScanRange[100] = { 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02};
 ...
 ///==== EB ====
  ...
  int BIN = 200;
 //== fbrem ==
  double MinScanRange[100] = {-0.04, -0.10, -0.10, -0.10, -0.11, -0.12, -0.06, -0.06, -0.06, -0.06, -0.06, -0.06, -0.06, -0.06, -0.06};
  double MaxScanRange[100] = { 0.01, 0.02, 0.02, 0.03, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02};
 ...
 for (int iMom=0; iMom<2; iMom++){
  ...
  for (int iNUM = 0; iNUM < NUM; iNUM++){
   ...
   std::cerr << " NAME = " << NAME.c_str() << " VARIABLE = " << VARIABLE << " VALUE_MIN[" << iNUM << "] = " << VALUE_MIN[iNUM] << " VALUE_MAX[" << iNUM << "] = " << VALUE_MAX[iNUM] << " MOMENTUM[" << iMom << "] = " << MOMENTUM[iMom] << std::endl;
   
   std::ostringstream ostr_NameFile;
   ostr_NameFile << "out_WenuEnScale_" << NAME.c_str() << "_" << VARIABLE << "_" << VALUE_MIN[iNUM] << "_" << VALUE_MAX[iNUM] << "_" << MOMENTUM[iMom] << ".root";
   ...
   if (fitStatus==4000){
    ...
    grResult[iMom]->SetPoint(iPoint,(VALUE_MIN[iNUM]+VALUE_MAX[iNUM])/2.,-b/(2*a));
    
    if (NAME_TEST=="Graph;2") grResult[iMom]->SetPointError(iPoint,(VALUE_MAX[iNUM]-VALUE_MIN[iNUM])/2.,1/sqrt(a));
    else grResult[iMom]->SetPointError(iPoint,(VALUE_MAX[iNUM]-VALUE_MIN[iNUM])/2.,1/sqrt(2*a));
  ...
  grResult[iMom]->GetXaxis()->SetTitle(VARIABLE);
...
///==== previous version: 1pb-1 data ==== worked well ====
...
// //   double RANGE_MAX[100] = {1.20,1.20,1.20,1.20,1.20,1.20,1.20,1.20,1.20};
//  
// //  char MOMENTUM[1000] = {"pIn"};
//    char MOMENTUM[1000] = {"pOut"};
//  
//  
//  
//  
...
//  for (int iNUM = 0; iNUM < NUM; iNUM++){
//   std::cerr << " iNUM = " << iNUM << " : " << NUM << std::endl;
//   std::cerr << " NAME = " << NAME.c_str() << " VARIABLE = " << VARIABLE << " VALUE_MIN[" << iNUM << "] = " << VALUE_MIN[iNUM] << " VALUE_MAX[" << iNUM << "] = " << VALUE_MAX[iNUM] << " MOMENTUM = " << MOMENTUM << std::endl;
//   std::ostringstream ostr_NameFile;
//   ostr_NameFile << "out_WenuEnScale_" << NAME.c_str() << "_" << VARIABLE << "_" << VALUE_MIN[iNUM] << "_" << VALUE_MAX[iNUM] << "_" << MOMENTUM << ".root";
//   
//   TString nameFile = Form("temp_EoP/%s",ostr_NameFile.str().c_str());
//   
//   std::cerr << " File = " << nameFile.Data() << std::endl;
//   TFile* file = TFile::Open(nameFile.Data());
//   TGraph* grLL = (TGraph*) file->Get(NAME_TEST.c_str());
...
//    iPoint++;
//    
//    grResult->SetPoint(iPoint,(VALUE_MIN[iNUM]+VALUE_MAX[iNUM])/2.,-b/(2*a));
//   
//    if (NAME_TEST=="Graph;2") grResult->SetPointError(iPoint,(VALUE_MAX[iNUM]-VALUE_MIN[iNUM])/2.,1/sqrt(a));
//    else grResult->SetPointError(iPoint,(VALUE_MAX[iNUM]-VALUE_MIN[iNUM])/2.,1/sqrt(2*a));
//    
//    
//    
...
//  grResult->Draw("AP");
// //  grResult->GetXaxis()->SetTitle("E5x5 / E_{SC}");
//  grResult->GetXaxis()->SetTitle(VARIABLE);
