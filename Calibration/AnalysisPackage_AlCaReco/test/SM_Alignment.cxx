...
//  TF1* Barrel = new TF1("Barrel","-1./1.290*(cos(THETA)*[0]-sin(THETA)*[1])");
...
//  double MINETA = 0.00;
 ...
 TF1* Barrel = new TF1("Barrel","-1./1.290*(cos(2*atan(exp(-x)))*[0]-sin(2*atan(exp(-x)))*[1])",MINETA,MAXETA);
...
//  TFile* FileIn = new TFile("output_Ntuple/AlCaReco_Results_new_NoMETCut.root","READ");
 ...
 cSummed->cd(1);
 TH2F* DEtaVsPhi_Plus = new TH2F("DEtaVsPhi_Plus","DEtaVsPhi_Plus",NBIN_PHI,-3.14159265-0.175,3.14159265-0.175,NBIN_DETA,MINDETA,MAXDETA);
 myTree->Draw("DeltaEtaIn:(phiSC)>>DEtaVsPhi_Plus","etaSC>0 && abs(etaSC)<1.5","colz");
 ...
 cSummed->cd(2);
 TH2F* DEtaVsPhi_Minus = new TH2F("DEtaVsPhi_Minus","DEtaVsPhi_Minus",NBIN_PHI,-3.14159265-0.175,3.14159265-0.175,NBIN_DETA,MINDETA,MAXDETA);
 myTree->Draw("DeltaEtaIn:(phiSC)>>DEtaVsPhi_Minus","etaSC<0 && abs(etaSC)<1.5","colz");
...
//  double Phi_SM_RMS[36];
 for (int iSM = 0; iSM<36; iSM++){
  if (iSM<18) Phi_SM_Mean[iSM] = (2*3.14159265*(iSM+0.5)/18.-0.175);
  else Phi_SM_Mean[iSM] = (2*3.14159265*(iSM-18+0.5)/18.-0.175);
//   Phi_SM_RMS[iSM] = 2*3.14159265 / 18.;
 }
//  TCanvas* cGr_DPHI_DETA_vs_PHI = new TCanvas ("cGr_DPHI_DETA_vs_PHI","cGr_DPHI_DETA_vs_PHI",800,1200);
//  cGr_DPHI_DETA_vs_PHI->Divide(2,2);
 ...
 double DEta_EBPlus_Mean[36];
 double DEta_EBPlus_RMS[36];
 double DEta_EBMinus_Mean[36];
 double DEta_EBMinus_RMS[36];
 ...
 for (int iSM = 0; iSM<36; iSM++){
  ...
  TString cut;
  if (iSM<18) {
   if (iSM<9) cut = Form("%s && etaSC>0 && abs(etaSC)<1.5 && phiSC>(3.14159265*%d/9.-0.175) && phiSC<(3.14159265*(%d+1)/9.-0.175)",genCut.Data(),iSM,iSM);
   else if (iSM == 9) cut = Form("%s && etaSC>0 && abs(etaSC)<1.5 && (phiSC>(3.14159265-0.175) || phiSC<(0.175-3.14159265))",genCut.Data());
   else cut = Form("%s && etaSC>0 && abs(etaSC)<1.5 && phiSC>(3.14159265*%d/9.-0.175-2*3.14159265) && phiSC<(3.14159265*(%d+1)/9.-0.175-2*3.14159265)",genCut.Data(),iSM,iSM);
  }
  else {
   if (iSM<27) cut = Form("%s && etaSC<0 && abs(etaSC)<1.5 && phiSC>(3.14159265*%d/9.-0.175) && phiSC<(3.14159265*(%d+1)/9.-0.175)",genCut.Data(),iSM-18,iSM-18);
   else if (iSM == 9) cut = Form("%s && etaSC<0 && abs(etaSC)<1.5 && (phiSC>(3.14159265-0.175) || phiSC<(0.175-3.14159265))",genCut.Data());
   else cut = Form("%s && etaSC<0 && abs(etaSC)<1.5 && phiSC>(3.14159265*%d/9.-0.175-2*3.14159265) && phiSC<(3.14159265*(%d+1)/9.-0.175-2*3.14159265)",genCut.Data(),iSM-18,iSM-18);
  }
  std::cout << " draw:cut = " << draw.Data() << "  :  " << cut.Data() << std::endl;
  myTree->Draw(draw,cut);
  DEta_Mean[iSM] = DEta[iSM]->GetMean();
  DEta_RMS[iSM] = DEta[iSM]->GetRMS() / sqrt(DEta[iSM]->GetEffectiveEntries());
  
  if (iSM<18) DEta_EBPlus_Mean[iSM] = DEta_Mean[iSM];
  else  DEta_EBMinus_Mean[iSM-18] = DEta_Mean[iSM];
  if (iSM<18) DEta_EBPlus_RMS[iSM] = DEta_RMS[iSM];
  else  DEta_EBMinus_RMS[iSM-18] = DEta_RMS[iSM];
 ...
 cGr->cd(2);
 TGraphErrors* gr = new TGraphErrors(36,phi_Mean,DEta_Mean,phi_RMS,DEta_RMS);
...
//  gr_EBPlus_DETA_vs_PHI->DrawClone("AP");
//  gPad->SetGrid();
...
//  gr_EBMinus_DETA_vs_PHI->DrawClone("AP");
//  gPad->SetGrid();
 ...
 for (int iSM = 0; iSM<36; iSM++){
  ...
  Phi_SM_Mean[iSM] = (2*3.14159265*(iSM+0.5)/18.-0.175);
...
//   TString draw = Form("DeltaEtaIn:abs(etaSC)>>%s",name.Data());
  ...
  Barrel->SetParameter(0,0);
  Barrel->SetParameter(1,0);
  
  
//   profile_DEta[iSM]->Fit("Barrel");
...
//    std::cout << " vEnergy[" << iEvt << "] = " << vEnergy[iEvt] <<  " vEta[" << iEvt << "] = " << vEta[iEvt] <<  " vDEta[" << iEvt << "] = " << vDEta[iEvt] << " : " << mylist->GetN() << std::endl;
  ...
  grDEta_vs_eta[iSM] -> Fit("Barrel","QM","",MINETA,MAXETA);
...
//   myTree->UnbinnedFit("Barrel","DeltaPhiIn",cut.Data());
  
  
  
  std::cout << " iSM = " << iSM << " DR = " << Barrel->GetParameter(0) << " +/- " << Barrel->GetParError(0) << std::endl;
  std::cout << " iSM = " << iSM << " DZ = " << Barrel->GetParameter(1) << " +/- " << Barrel->GetParError(1) << std::endl;
  std::cout << " ===================================================== " << std::endl;
  
  gPad->Update();
  DEtaVsEta[iSM]->Draw("colz");
  profile_DEta[iSM]->Draw("same");
  Barrel->DrawClone("same");
  
  if (iSM<18) iSM_SM_Mean[iSM] = iSM+1;
  else iSM_SM_Mean[iSM] = -(iSM-17);
  iSM_SM_RMS[iSM] = 0;
  DR_SM_Mean[iSM] = Barrel->GetParameter(0);
  DR_SM_RMS[iSM] = Barrel->GetParError(0);
  if (iSM<18) DZ_SM_Mean[iSM] = Barrel->GetParameter(1);
  else DZ_SM_Mean[iSM] = - Barrel->GetParameter(1);
  DZ_SM_RMS[iSM] = Barrel->GetParError(1);
  
  
  DX_SM_Mean[iSM] = (cos(Phi_SM_Mean[iSM]) * DR_SM_Mean[iSM] - 1.290 * sin(Phi_SM_Mean[iSM]) * DPhi_SM_Mean[iSM]);
  DX_SM_RMS[iSM] = sqrt(cos(Phi_SM_Mean[iSM]) * DR_SM_Mean[iSM] * cos(Phi_SM_Mean[iSM]) * DR_SM_Mean[iSM] + 1.290 * sin(Phi_SM_Mean[iSM]) * DPhi_SM_Mean[iSM] * 1.290 * sin(Phi_SM_Mean[iSM]) * DPhi_SM_Mean[iSM]);
  DY_SM_Mean[iSM] = (sin(Phi_SM_Mean[iSM]) * DR_SM_Mean[iSM] + 1.290 * cos(Phi_SM_Mean[iSM]) * DPhi_SM_Mean[iSM]);
  DY_SM_RMS[iSM] = sqrt(sin(Phi_SM_Mean[iSM]) * DR_SM_Mean[iSM] * sin(Phi_SM_Mean[iSM]) * DR_SM_Mean[iSM] + 1.290 * cos(Phi_SM_Mean[iSM]) * DPhi_SM_Mean[iSM] * 1.290 * cos(Phi_SM_Mean[iSM]) * DPhi_SM_Mean[iSM]);
...
//  gr_EBPlus_DPHI_vs_PHI->DrawClone("AP");
//  gPad->SetGrid();
...
//  gr_EBMinus_DPHI_vs_PHI->DrawClone("AP");
//  gPad->SetGrid();
 ...
 ofstream outFile;
 outFile.open ("AlignmentElectron.txt");
 outFile << "#SM id dx errdx dy errdy dz errdz " << std::endl;
 for (int iSM = 0; iSM<36; iSM++){
  if (iSM<18) outFile << "EB+" << iSM+1;
  else outFile << "EB-" << iSM-18;
  outFile << " " << setfill(' ') << setw(10) << setprecision(5) << 1000*DX_SM_Mean[iSM] << " " << setfill(' ') << setw(10) << setprecision(5) << 1000*DX_SM_RMS[iSM];
  outFile << " " << setfill(' ') << setw(10) << setprecision(5) << 1000*DY_SM_Mean[iSM] << " " << setfill(' ') << setw(10) << setprecision(5) << 1000*DY_SM_RMS[iSM];
  outFile << " " << setfill(' ') << setw(10) << setprecision(5) << 1000*DZ_SM_Mean[iSM] << " " << setfill(' ') << setw(10) << setprecision(5) << 1000*DZ_SM_RMS[iSM];
  outFile << std::endl;
 }
 outFile.close(); 
