...
c++ -o TT_RECO_5 `root-config --cflags --glibs` -lMinuit -lGenVector TT_RECO_5.cpp dict.cpp treeReader.cc
*/
...
void Update(std::vector<double>* KK_In){
 ...
 for (int i=0; i<nParametri; i++){
  ...
  if (Down->at(i) > 0){
   std::cerr << " 1 = " << (Up->at(i) / Down->at(i)) << std::endl;
...
int main (int argc, char ** argv)
{
 ...
 TFile outFile("output/outFile_KK_5.root","RECREATE");
