...
c++ -o TT_RECO_5_Alfa `root-config --cflags --glibs` -lMinuit -lGenVector TT_RECO_5_Alfa.cpp dict.cpp treeReader.cc
*/
...
//---- Update ----
void Update(std::vector<double>* KK_In, int alfa){
 ...
 for (int i=0; i<n; i++){
  ...
  double Up_temp = 80.0 * 80.0 / (M_temp * M_temp * KK_In->at(iPt1 * intEta + iEta1)* KK_In->at(iPt2 * intEta + iEta2));
  Up_temp = sqrt (Up_temp);
  double newUp = Up_temp;
  for (int ll=1; ll< alfa; ll++) {
   Up_temp = Up_temp * newUp;
  }
  
  double Down_temp = KK_In->at(iPt1 * intEta + iEta1)* KK_In->at(iPt2 * intEta + iEta2);
  Down_temp = sqrt (Down_temp);
  double newDown = Down_temp;
  for (int ll=1; ll< (alfa/2); ll++) {
...
int main (int argc, char ** argv)
{
 ...
 int alfa = 1;
 if (argc >= 6) alfa = atoi(argv[5]);
 std::ostringstream oss_name;
 oss_name << "output/outFile_KK_5_" << alfa << ".root";
 std::string nameOutFile(oss_name.str());

 TFile outFile(nameOutFile.c_str(),"RECREATE");
