...
int main (int argc, char** argv)
{
  ...
  if (argc != 2)
  {
    std::cerr << ">>>>> ttbarPreselection::usage: " << argv[0] << " configFileName" << std::endl ;
    return 1 ;
  }
...
//      std::cout << step_name.str () << std::endl ;
  ...
  // Loop over events
  std::cout << ">>>>> ttbarPreselection::Read " << chain -> GetEntries () << " entries" << std::endl ;  
  for (int entry = 0 ; entry < chain -> GetEntries () ; ++entry)
    {
      reader.GetEntry (entry) ;
      if ( (entry%entryMODULO) == 0) std::cout << ">>>>> ttbarPreselection::GetEntry " << entry << std::endl ;   
