...
c++ -o TT_RECO_6 `root-config --cflags --glibs` -lMinuit -lGenVector TT_RECO_6.cpp dict.cpp treeReader.cc
*/
...
void Update(std::vector<double>* KK_In){
 ...
 for (int i=0; i<nParametri; i++){
  if (CorrH->at(i).GetEntries() > 10){
   ...
   if (chisqdf < 5) { 
    KK_In->at(i) = (80.0 / mean) * KK_In->at(i);
//     KK_In->at(i) = (80.0 / mean) * (80.0 / mean) * KK_In->at(i);
//     std::cerr << " (80.0 / mean) * (80.0 / mean) = " << (80.0 / mean) * (80.0 / mean) << std::endl;
   }
  }
 }
...
int main (int argc, char ** argv)
{
 ...
 TFile outFile("output/outFile_KK_6.root","RECREATE");
