...
c++ -o TT_RECO_4 `root-config --cflags --glibs` -lMinuit -lGenVector TT_RECO_4.cpp dict.cpp treeReader.cc
*/
...
void Update(std::vector<double>* KK_In){
 ...
 for (int i=0; i<n; i++){
  ...
  double Up_temp = M_temp * M_temp * KK_In->at(iPt1 * intEta + iEta1);
  ...
  if ((iPt2 * intEta + iEta2) == (iPt1 * intEta + iEta1)) continue; //---- if the same, use one time!
  
  Up_temp = M_temp * M_temp * KK_In->at(iPt2 * intEta + iEta2);
 ...
 for (int i=0; i<nParametri; i++){
  ...
  if (Down->at(i) > 0){
   std::cerr << "   Down[" << i << "] = " << Down->at(i) << "   Up[" << i << "] = " << Up->at(i) << " 1 = " << 80.0 * 80.0 * (Up->at(i) / Down->at(i)) << std::endl;
   KK_In->at(i) = 80.0 * 80.0 * (Up->at(i) / Down->at(i)) ; //* KK_In->at(i);
  }
  else {
...
int main (int argc, char ** argv)
{
 ...
 TFile outFile("output/outFile_KK_4.root","RECREATE");
