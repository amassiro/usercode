...
c++ -o TT_RECO_2 `root-config --cflags --glibs` -lMinuit -lGenVector TT_RECO_2.cpp dict.cpp treeReader.cc
*/
...
void Function(int& npar, double* d, double& r, double par[], int flag){
 ...
 for (int i=0; i<n; i++){
  ...
  M_temp = M_temp * sqrt(par[iPt1 * intEta + iEta1] * par[iPt2 * intEta + iEta2]);
  result += (M_temp - 80.0) * (M_temp - 80.0);  
 }
 r = result;
}
...
int main (int argc, char ** argv)
{
 ...
 TFile outFile("output/outFile_KK_2.root","RECREATE");
 ...
 double* KK = new double [nParametri];
 ...
 std::cerr << std::endl << "*** Minuit ***" << std::endl;
 
 double par[nParametri];
 double stepSize[nParametri];
 double minVal[nParametri];
 double maxVal[nParametri];
 std::string parName[nParametri];
 ...
 std::cerr << std::endl << "*** Minuit::Migrad ***" << std::endl;
 minuit.Migrad();
 std::cerr << std::endl << "*** Minuit:: end Migrad ***" << std::endl;
 ...
 for (int ii=0; ii< nParametri; ii++) {
  KK[ii] = outParametri[ii];
 }
 
 for (int iEta=0; iEta<intEta; iEta++){
  for (int iPt=0; iPt<intPt; iPt++){
   hKK.SetBinContent(iPt+1,iEta+1,KK[iPt * intEta + iEta]);
  }
 }
 ...
 for (int iEvent = 0 ; iEvent < nentries ; ++iEvent)
 {
  ...
  if (matched > 0){
   if (matched == 2){
    ...
    M_temp = M_temp * sqrt(KK[iPt1 * intEta + iEta1] * KK[iPt2 * intEta + iEta2]);    
    M_RECO_AFTER.Fill(M_temp) ;
    t_M_Reco = M_temp;    
    t_Eta_Reco->push_back(jets->at(matchIt.at(0)).Eta());
    t_Eta_Reco->push_back(jets->at(matchIt.at(1)).Eta());
    t_pT_Reco->push_back(jets->at(matchIt.at(0)).Pt() * KK[iPt1 * intEta + iEta1]);
    t_pT_Reco->push_back(jets->at(matchIt.at(1)).Pt() * KK[iPt2 * intEta + iEta2]); 
    
    if (fabs(mcF1_fromV1_pdgId->at(0)) <= 4 && fabs(mcF1_fromV2_pdgId->at(0)) >= 11 ){
     PtOverPt_RECOMC_AFTER.Fill(jets->at(matchIt.at(0)).Pt() / mcF_fromV1->at(0).Pt() * KK[iPt1 * intEta + iEta1] );
     PtOverPt_RECOMC_AFTER.Fill(jets->at(matchIt.at(1)).Pt() / mcF_fromV1->at(1).Pt() * KK[iPt2 * intEta + iEta2] );
     t_pT_MC->push_back(mcF_fromV1->at(0).Pt());
     t_pT_MC->push_back(mcF_fromV1->at(1).Pt());
    }
    else {
     PtOverPt_RECOMC_AFTER.Fill(jets->at(matchIt.at(0)).Pt() / mcF_fromV2->at(0).Pt() * KK[iPt1 * intEta + iEta1]);
     PtOverPt_RECOMC_AFTER.Fill(jets->at(matchIt.at(1)).Pt() / mcF_fromV2->at(1).Pt() * KK[iPt2 * intEta + iEta2]);
     t_pT_MC->push_back(mcF_fromV2->at(0).Pt());
     t_pT_MC->push_back(mcF_fromV2->at(1).Pt());
    }
    t_Cycle_num = 1; //---- after
 ...
 for (int iEvent = nentries ; iEvent < nentriesTOT ; ++iEvent)
 {
  ...
  if (matched > 0){
   if (matched == 2){
    ...
    t_Eta_Reco->push_back(jets->at(matchIt.at(1)).Eta());    
    if (fabs(mcF1_fromV1_pdgId->at(0)) <= 4 && fabs(mcF1_fromV2_pdgId->at(0)) >= 11 ){
     PtOverPt_RECOMC_INDIP_AFTER.Fill(jets->at(matchIt.at(0)).Pt() / mcF_fromV1->at(0).Pt() * KK[iPt1 * intEta + iEta1] );
     PtOverPt_RECOMC_INDIP_AFTER.Fill(jets->at(matchIt.at(1)).Pt() / mcF_fromV1->at(1).Pt() * KK[iPt2 * intEta + iEta2] );
     ...
     PtOverPt_RECOMC_INDIP_AFTER.Fill(jets->at(matchIt.at(0)).Pt() / mcF_fromV2->at(0).Pt() * KK[iPt1 * intEta + iEta1]);
     PtOverPt_RECOMC_INDIP_AFTER.Fill(jets->at(matchIt.at(1)).Pt() / mcF_fromV2->at(1).Pt() * KK[iPt2 * intEta + iEta2]);
     PtOverPt_RECOMC_INDIP.Fill(jets->at(matchIt.at(0)).Pt() / mcF_fromV2->at(0).Pt());
     PtOverPt_RECOMC_INDIP.Fill(jets->at(matchIt.at(1)).Pt() / mcF_fromV2->at(1).Pt());
     t_pT_MC->push_back(mcF_fromV2->at(0).Pt());
     t_pT_MC->push_back(mcF_fromV2->at(1).Pt());     
    }
    t_Cycle_num = 0; //---- before
