...
int main(int argc, char** argv)
{
 ...
 std::string inputFileCalibrationConstants = gConfigParser -> readStringOption("Input::inputFileCalibrationConstants");
 
 std::ifstream inputFileCalibration(inputFileCalibrationConstants.c_str());
 std::string buffer;
 
 if(!inputFileCalibration.is_open())
 {
  std::cerr << "** ERROR: Can't open '" << inputFileCalibration << "' for input" << std::endl;
  return false;
 }
 
 while(!inputFileCalibration.eof())
 {
  double ptMin;
  double ptMax;
  double etaMin;
  ...
  getline(inputFileCalibration,buffer);
  ...
  if (buffer != ""){
   ...
   ///==============================================================
   myJetCalibrator.SetKK(KK_temp,eta,pt);
   myJetCalibrator.SetKKErr(KKErr_temp,eta,pt);
   ///==============================================================
  }
 } 
