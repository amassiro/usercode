 ...
 if (creator == 1){
  ...
  for(int iEvent = entryMIN ; iEvent < entryMAX ; ++iEvent)
  {
   ...
   if ( mcF_fromVandB->size() == 4 ){//---- only if at MC level found bb + qq' + (lnu) ----
    ...
    if (matched == 4){
     double doSig_doBkg = myrand.Uniform(-1,1); // <0 == bkg, >=0 == sig    
  ...
  if (kindAlgo == 0){
   ...
   //   TString weightfile = dir + prefix + "_" + TString(additionalInputFileName) + ".weights.xml";  
   TString weightfile = Form(additionalInputFileName.c_str());
   
   // methodName = "BDT";
   // weightfile = "weights/TMVAnalysis_BDT.weights.xml";
   ...
   for(int iEvent = entryMIN ; iEvent < entryMAX ; ++iEvent)
   {
    ...
    if ( mcF_fromVandB->size() == 4 ){//---- only if at MC level found bb + qq' + (lnu) ----
     ...
     if (matched == 4){
      ...
      double value_b = -100000000;
      double value_q = -100000000;
	...
	jets_combinedSecondaryVertexMVABJetTags_RECO_b2 = jets_combinedSecondaryVertexMVABJetTags->at(b1);
       ...
       double value_b_temp = jets_trackCountingHighEffBJetTags_RECO_b1 + jets_trackCountingHighEffBJetTags_RECO_b2;
       double value_q_temp = pT_RECO_q1 + pT_RECO_q2;
       
       ///=====================================
       ///===== selection cryterion ===========
       if (DR_RECO_qq >= 1.5){ ///=> look at plot!
	if (value_b_temp >= value_b) {
	 if (value_b_temp == value_b){
	  if (value_q_temp > value_q){
	   numCombination = iComb;
	   value_b = value_b_temp;
	   value_q = value_q_temp;
	  }
	 }
	 else {
	  numCombination = iComb;
      ...
      if (numCombination != -1){
       n_matched = CalculateMatch(matchIt,combinations.at(numCombination));
       n_jets = jets->size();
       outTree.Fill();     
      }
     }
