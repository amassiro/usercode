...
// char    inputDirectory[1000] = {"/gwteraz/users/amassiro/VBF/Summer11_25July_MC_PATLatinos/"};
 ...
 for (int iFile = 0; iFile < nFile; iFile++) {
  ...
  std::ofstream myfile;
  char nameFile[1000];
  sprintf(nameFile,"test/Latinos/dir_cfg_skimmed_MC_Summer11/NtupleProducerPATtoNT_%s_%d.sh",nameSample[iSample],iFile+1);
  ...
  myfile << "./bin/NtupleProducerFromPATtoNT.exe    test/Latinos/dir_cfg_skimmed_MC_Summer11/NtupleProducerPATtoNT_" << nameSample[iSample] << "_" << iFile+1 << ".cfg" << std::endl;
  myfile << std::endl;
  myfile.close(); 
