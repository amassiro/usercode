{
 ...
 char outputDirectory[1000] = {"/gwteraz/users/amassiro/VBFPlainRoot/Latinos_Aug112011_Summer11/"};
...
// char    inputDirectory[1000] = {"/gwteraz/users/amassiro/VBF/Summer11_25July_MC_PATLatinos/"};
 char    inputDirectory[1000] = {"/gwteraz/users/amassiro/VBF/Summer11_Aug112011_MC_PATLatinos/"};  
  
 char *nameSample[1000];
 char *nameHumanReadable[1000];
 char* xsection[1000];
 
 char nameFileIn[1000] = {"test/Latinos/samples_MC_Summer11.txt"};
 
 int numberOfSamples =  ReadFile(nameFileIn,nameSample,nameHumanReadable, xsection);

 TString toDoShell;
 
 toDoShell= Form ("mkdir test/Latinos/dir_cfg_skimmed_MC_Summer11");
 ...
 for (int iFile = 0; iFile < nFile; iFile++) {
   ...
   std::ofstream myfile;
  char nameFile[1000];
  sprintf(nameFile,"test/Latinos/dir_cfg_skimmed_MC_Summer11/NtupleProducerPATtoNT_%s_%d.cfg",nameSample[iSample],iFile+1);
