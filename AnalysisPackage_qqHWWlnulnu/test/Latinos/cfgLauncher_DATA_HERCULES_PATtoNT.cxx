...
//  nFile -= 1; ///----> there is the source file too!
 ...
 for (int iFile = 0; iFile < nFile; iFile++) {
  ...
  std::ofstream myfile;
  char nameFile[1000];
  sprintf(nameFile,"test/Latinos/dir_cfg_skimmed_DATA/NtupleProducerPATtoNT_%s_%d.sh",nameSample[iSample],iFile);
  ...
  myfile << "./bin/NtupleProducerFromPATtoNT.exe    test/Latinos/dir_cfg_skimmed_DATA/NtupleProducerPATtoNT_" << nameSample[iSample] << "_" << iFile << ".cfg" << std::endl;
  myfile << std::endl;
  myfile.close(); 
