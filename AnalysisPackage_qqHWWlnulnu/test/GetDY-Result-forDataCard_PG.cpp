/* how to use me
 ...
 for i in 1 2 3 4 5 6 7 ; do \
  ./bin/GetDY-Result-forDataCard_PG.exe out_DYRinout_Latinos_30Aug1800-ForResult.root $i ; \
 done > output_DY_spring.txt
...
//void GetDY_Result_forDataCard_PG(TString nameInFileRoot, int WorkingPoint = 1){
// 

int main (int a, char ** b)
{
 
 TString nameInFileRoot = b[1] ;
 int WorkingPoint = atoi (b[2]) ;
...
/*
    $m_H                          $ &
    $N_{\mu\mu}^{\textrm{in, CR}} $ &
    $N_{ee}^{\textrm{in, CR}}     $ &
    $N_{\mu\mu}^{\textrm{out, CR}}$ &
    $N_{ee}^{\textrm{out, CR}}    $ &
    $N_{\mu{}e}^{\textrm{out, CR}}$ &
    $N_{e\mu}^{\textrm{out, CR}}  $ &
    $R_{\mu\mu}                   $ &
    $R_{ee}                       $ \\
    
    suppongo A = out of peak, B = in peak    
...
*/ 
 ...
 std::cout << " PGT1 | " << WorkingPoint << " & "
           << Nmumu_CR_B << " $\\pm$ " << errNmumu_CR_B << " & " 
           << Nee_CR_B   << " $\\pm$ " << errNee_CR_B   << " & " 
           << Nmumu_CR_A << " $\\pm$ " << errNmumu_CR_A << " & " 
           << Nee_CR_A   << " $\\pm$ " << errNee_CR_A   << " & " 
           << Nmue_CR_A  << " $\\pm$ " << errNmue_CR_A  << " & " 
           << Nemu_CR_A  << " $\\pm$ " << errNemu_CR_A  << " & " 
           << (Nmumu_CR_A - sqrt(Nmumu_CR_A / Nee_CR_A * Nemu_CR_A * Nmue_CR_A)) / (Nmumu_CR_B - sqrt(Nmumu_CR_B / Nee_CR_B * Nemu_CR_B * Nmue_CR_B))   << " & " 
           << (Nee_CR_A - sqrt(Nee_CR_A / Nmumu_CR_A * Nemu_CR_A * Nmue_CR_A)) / (Nee_CR_B - sqrt(Nee_CR_B / Nmumu_CR_B * Nemu_CR_B * Nmue_CR_B))       << " \\\\ " << std::endl ;

 std::cout << " ****************************** " << std::endl;

/* 
    $m_H                          $ &
    $N_{\mu\mu}^{\textrm{in, SR}} $ &
    $N_{ee}^{\textrm{in, SR}}     $ &
    $N_{\mu{}e}^{\textrm{out, SR}}$ &
    $N_{e\mu}^{\textrm{out, SR}}  $ &
*/
 ...
 std::cout << " PGT2 | " << WorkingPoint << " & "
           << Nmumu_SR_B << " $\\pm$ " << errNmumu_SR_B << " & " 
           << Nee_SR_B   << " $\\pm$ " << errNee_SR_B   << " & " 
           << Nmue_SR_B  << " $\\pm$ " << errNmue_SR_B  << " & " 
           << Nemu_SR_B  << " $\\pm$ " << errNemu_SR_B  << " \\\\ " << std::endl;
 ...
 std::cout << " PGT3 | " << WorkingPoint << " & "
           << Nee_Result   << " $\\pm$ " << errNee_Result << " & "
           << Nmumu_Result << " $\\pm$ " << errNmumu_Result  << " \\\\ \n" ;
