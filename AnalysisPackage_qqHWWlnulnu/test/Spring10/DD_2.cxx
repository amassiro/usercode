void DD_2( TString nameSampleIN = "" , double MinCutValue){
...
///================================================================================================
 ...
 signal_background[iSample]->SetFillColor(kRed);
 signal_background[iSample]->SetFillStyle(3001);
 signal_background[iSample]->SetLineWidth(1.);
...
//  TH1F* h1Distro = new TH1F("h1Distro","h1Distro",BinX,MinX,MaxX);
 ...
 htempTotal->Scale(weights[iSample]);
 htempTotal->Draw();
 ...
 signal_background[iSample]->Draw(toDraw,toCut,"");
 htempLeft->Scale(weights[iSample]);
 ...
 signal_background[iSample]->Draw(toDraw,toCut,"");
 std::cerr << " toCut = " << toCut << std::endl;
 htempRight->Scale(weights[iSample]);
...
//  htempRight->Smooth(1);
 ...
 for (int iLumi = 0; iLumi < nLumi; iLumi++){
  ...
  int numBins = (weights[iSample] * (iLumi+1) * deltaLumi * htempTotal->GetEntries()) / 10;
  ...
  for (int iTest = 0; iTest < nTest; iTest++){
   yRatio[iLumi] = 0;
   int numEvents = gRandom->Poisson(weights[iSample] * (iLumi+1) * deltaLumi * htempTotal->GetEntries()); ///==== Poisson ====
  ...
  xLumi[iLumi] = (weights[iSample] * (iLumi+1) * deltaLumi) * htempTotal->GetEntries();
  errxLumi[iLumi] = 0;
  if (!(iLumi%10)){
   std::cerr << "yRatio[" << iLumi << "] = " << yRatio[iLumi] << " +/-" << erryRatio[iLumi] << " : " << xLumi[iLumi] << std::endl;
  }
  hRatio[iLumi]->SetLineColor(kOrange);
  hRatio[iLumi]->SetFillColor(kOrange);
