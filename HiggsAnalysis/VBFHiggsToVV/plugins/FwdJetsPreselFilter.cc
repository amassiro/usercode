...
//
// Package:    GenFilters
// Class:      FwdJetsPreselFilter
// 
/**\class FwdJetsPreselFilter FwdJetsPreselFilter.cc GeneratorInterface/GenFilters/src/FwdJetsPreselFilter.cc
...
// $Id: FwdJetsPreselFilter.cc,v 1.2 2009/03/26 23:03:28 yarba Exp $
//
//
...
FwdJetsPreselFilter::FwdJetsPreselFilter (const edm::ParameterSet& iConfig) : 
...
FwdJetsPreselFilter::~FwdJetsPreselFilter () {}


// ----------------------------------------------------------------


void FwdJetsPreselFilter::beginJob (const edm::EventSetup&) 
{
}
...
void FwdJetsPreselFilter::endJob () 
{
    ...
    const edm::EventSetup& iSetup) 
{
  ...
  for (int iGj = 0 ; iGj < jetsHandle->size () ; ++iGj)
    {
      if (jetsHandle->at (iGj).pt () > m_ptThres) goodJets.push_back (iGj) ;    
    }
  if (goodJets.size () < m_nJets) return false ;

  //PG loop over jet pairs
  for (int iGj = 0 ; iGj < goodJets.size () - 1 ; ++iGj)
    for (int jGj = iGj + 1 ; jGj < goodJets.size () ; ++jGj)
      {
